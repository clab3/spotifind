...
class SpotifyApiClient {
  ...
  async init(): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch acßcess token from Spotify web api');
    }
  ...
  async getArtistAlbums(artistSpotifyId: string): Promise<AlbumInfo[]> {
    if (!this.accessToken) {
      throw new Error('SpotifyApiClient: Access token not set, cannot call getArtistAlbums()');
    }
  ...
  async getAlbum(albumSpotifyId: string): Promise<AlbumDetailInfo> {
    if (!this.accessToken) {
      throw new Error('SpotifyApiClient: Access token not set, cannot call getAlbum()');
    }
